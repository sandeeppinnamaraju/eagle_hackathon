...
export const Route = createFileRoute("/studies_/$studyId")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.studyId} G현 Study Overview G현 Flight Deck` },
  ...
  if (!study) {
    return (
      <main className="mx-auto max-w-[1600px] px-6 py-12 text-center">
        ...
        <Link to="/portfolio" className="mt-4 inline-block text-primary hover:underline">G煬 Back to Study Portfolio</Link>
      </main>
    );
  }
...
function buildMilestones(detail: StudyDetail): MilestoneRow[] {
  ...
  return [
    { code: "FSA",  label: "First Site Activated",     planned: fsaPlanned ? fmt(fsaPlanned) : "G현", actual: fsaActual ? fmt(fsaActual) : "G현" },
    { code: "FSFV", label: "First Subject First Visit", planned: detail.plannedFPI, actual: detail.actualFPI },
    { code: "LSFV", label: "Last Subject First Visit",  planned: detail.plannedLPI, actual: "G현" },
    { code: "DBL",  label: "Database Lock",             planned: dblPlanned ? fmt(dblPlanned) : "G현", actual: "G현" },
    { code: "RC",   label: "Report Complete",           planned: rcPlanned ? fmt(rcPlanned) : "G현", actual: "G현" },
  ];
}

function variance(planned: string, actual: string): { text: string; tone: "neutral" | "ok" | "warn" | "bad" } {
  if (!actual || actual === "G현") return { text: "Pending", tone: "neutral" };
  const p = parseDate(planned); const a = parseDate(actual);
  if (!p || !a) return { text: "G현", tone: "neutral" };
                ...
                return (
                  <tr key={r.code} className="border-t border-border">
                    <td className="px-3 py-2.5 align-top">
                      ...
                      <span className="text-muted-foreground"> G현 {r.label}</span>
                    </td>
                    <td className="px-3 py-2.5 align-top tabular-nums text-foreground">{r.planned || "G현"}</td>
                    <td className="px-3 py-2.5 align-top tabular-nums text-foreground">{r.actual || "G현"}</td>
