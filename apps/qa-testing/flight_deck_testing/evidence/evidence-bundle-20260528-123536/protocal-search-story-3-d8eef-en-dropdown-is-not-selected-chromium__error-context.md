...
Expected pattern: /\/protocol-search(\?mode=input)?$/i
Received string:  "https://spend-mhz-bufing-characteristics.trycloudflare.com/login"
Timeout: 10000ms

Call log:
  - Expect "toHaveURL" with timeout 10000ms
    23 × unexpected value "https://spend-mhz-bufing-characteristics.trycloudflare.com/login"

```

```yaml
- main:
  - text: Flight Deck Clinical Trials Intelligence
  - heading "Pilot every protocol with precision" [level=1]
  - paragraph: Flight Deck is the operating cockpit for clinical operations teams. Turn enrollment signals, site performance, and protocol risk into the decisions that move medicine forward — from first patient in to final readout.
  - list:
    - listitem:
      - paragraph: Unified portfolio
      - paragraph: Track every study, site, and milestone across regions in one live workspace.
    - listitem:
      - paragraph: Real-time signals
      - paragraph: Enrollment velocity, screen-fail rates, and risk scores updated as data arrives.
    - listitem:
      - paragraph: Protocol intelligence
      - paragraph: Search 100k+ protocols and benchmark designs against your therapeutic area.
  - heading "Sign in" [level=2]
  - paragraph: Enter your credentials to access Flight Deck.
  - text: Username
  - textbox "Username":
...
```ts
  ...
  13  |     const response = await page.goto(BASE_URL.replace(/\/$/, '') + PROTOCOL_SEARCH_PATH);
  ...
  34  |     await page.goto(BASE_URL, { waitUntil: 'domcontentloaded' });
  35  | }
  36  | 
...
> 42  |     await expect(page).toHaveURL(/\/protocol-search(\?mode=input)?$/i);
  ...
  135 |             await summaryInput.press('Enter').catch(() => {});
  136 |         }
  137 |         await page.waitForLoadState('networkidle').catch(() => {});
  138 | 
  139 |         // Results or empty state
  140 |         const resultsContainer = page.locator('article, table, [role="table"], [class*="card"]').first();
  141 |         const resultItems = page.locator('article, table tbody tr, [role="row"], [class*="card"]');
  142 |         const emptyState = page.getByText(/no results|no similar protocols|nothing found|empty|0 protocols matched/i).first();
```