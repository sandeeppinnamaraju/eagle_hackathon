...
Call log:
  - waiting for getByPlaceholder('Search by ID, title...').or(locator('input[placeholder*="Search"], input[type="search"], input').first()).first()
...
> 53  |     await search.press('Enter');
  ...
  115 |     } else {
  116 |       await expect(page.getByText(/0 loaded|loaded/i).first()).toBeVisible();
  117 |     }
  118 |   });
  119 | });
  120 | 
