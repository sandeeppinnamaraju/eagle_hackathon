...
def _normalize_pg_user(host: str, user: str) -> str:
    """Optionally normalize username to user@server for Azure PostgreSQL."""
    force_suffix = os.getenv("PGUSER_FORCE_AZURE_SUFFIX", "false").lower() in {"1", "true", "yes"}
    if not force_suffix:
        return user

    azure_suffix = ".postgres.database.azure.com"
    if azure_suffix in host and "@" not in user:
...
def _build_database_url() -> str:
    ...
    user = _normalize_pg_user(host, user)
