...
# Prefer apps/database/data; keep legacy fallback for older layouts.
PRIMARY_DATA_DIR = (DATABASE_DIR / "data").resolve()
LEGACY_DATA_DIR = (SCRIPT_DIR / "data").resolve()
DATA_DIR = PRIMARY_DATA_DIR if PRIMARY_DATA_DIR.exists() or not LEGACY_DATA_DIR.exists() else LEGACY_DATA_DIR
...
def insert_row(engine, table_name, row):
    tbl = Table(table_name, metadata, autoload_with=engine)
    with engine.begin() as conn:
        conn.execute(tbl.insert().values(**row))

def load_table(engine, sheet_name, schema, df):
    """Load: Insert new rows into the database, skip duplicates."""
    ...
    for _, row in df.iterrows():
        ...
        if pk_cols and all(row_dict.get(pk) is not None for pk in pk_cols):
            if row_exists(engine, table_name, pk_cols, row_dict):
                log_and_print(f"Duplicate row in {table_name} with PK {[(pk, row_dict[pk]) for pk in pk_cols]}", level="warning")
                skipped += 1
                continue
        try:
            insert_row(engine, table_name, row_dict)
            inserted += 1
        except SQLAlchemyError as e:
            log_and_print(f"Failed to insert row in {table_name}: {e}", level="error")
            skipped += 1
    log_and_print(f"{table_name}: Inserted {inserted}, Skipped {skipped}")
